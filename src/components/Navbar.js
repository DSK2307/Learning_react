...
export default function Navbar(props) {
    return (
        <nav
            className={`navbar bg-${props.mode} text-${props.mode === 'light' ? 'dark' : 'light'} navbar-${props.mode} px-5 py-3 flex justify-between items-center border-b-2 border-gray-600 mb-0`}
        >
            <Link className="text-2xl font-bold" to="/">{props.title}</Link>
